...
-(void)testObjectInitWithObjectTypeObject {
    ...
    RLMAssertThrowsWithReason([[DogExtraObject alloc] initWithValue:dog], @"Initialization with missing values should throw", @"dongs");
...
-(void)testCreateInRealmWithObjectLiterals {
    ...
    RLMAssertThrowsWithReason([CompanyObject createInDefaultRealmWithValue:invalidArray], @"Invalid sub-literal should throw", @"dongs");
    ...
    // missing properties
    RLMAssertThrowsWithReason([DogExtraObject createInDefaultRealmWithValue:dog], @"Initialization with missing values should throw", @"dongs");
...
- (void)testPrimaryKey {
    ...
    [PrimaryNullableIntObject createInDefaultRealmWithValue:(@{@"optIntCol": @2})];
    [PrimaryNullableIntObject createInDefaultRealmWithValue:@[NSNull.null]];
    RLMAssertThrowsWithReason([PrimaryNullableIntObject createInDefaultRealmWithValue:(@[@1])],
                              @"existing primary key value");
    RLMAssertThrowsWithReason([PrimaryNullableIntObject createInDefaultRealmWithValue:(@[NSNull.null])],
                              @"existing primary key value");
...
- (void)testCreateOrUpdate {
    ...
    RLMAssertThrowsWithReason([StringObject createOrUpdateInDefaultRealmWithValue:@[@"string"]],
                              @"'StringObject' does not have a primary key");
    RLMAssertThrowsWithReason([StringObject createOrUpdateInRealm:realm withValue:@[@"string"]],
                              @"'StringObject' does not have a primary key");
...
- (void)testObjectForKey {
    ...
    // FIXME: need model classes for these
